...
CREATE OR REPLACE FUNCTION transducer._position_INSERT_fn()
   ...
   ELSE
      ...
      INSERT INTO transducer._position_INSERT 
      SELECT dep_address, city, country FROM new;
      RETURN NEW;
   END IF;
END;  $$;
...
CREATE OR REPLACE FUNCTION transducer._empdep_INSERT_fn()
   ...
   ELSE
      INSERT INTO transducer._loop VALUES (-1);
      INSERT INTO transducer._empdep_INSERT 
      SELECT ssn, name, phone, email, dep_name, dep_address FROM new;
      RETURN NEW;
   END IF;
END;  $$;
...
CREATE OR REPLACE FUNCTION transducer._person_email_INSERT_fn()
   ...
   ELSE
      INSERT INTO transducer._loop VALUES (-1);
      INSERT INTO transducer._person_email_INSERT 
      SELECT ssn, email FROM new;
      RETURN NEW;
   END IF;
END;  $$;
...
CREATE OR REPLACE FUNCTION transducer._department_city_INSERT_fn()
   ...
   ELSE
      INSERT INTO transducer._loop VALUES (-1);
      INSERT INTO transducer._department_city_INSERT 
      SELECT dep_address, city FROM new;
      RETURN NEW;
   END IF;
END;  $$;
...
CREATE OR REPLACE FUNCTION transducer._city_country_INSERT_fn()
   ...
   ELSE
      INSERT INTO transducer._loop VALUES (-1);
      INSERT INTO transducer._city_country_INSERT 
      SELECT city, country FROM new;
      RETURN NEW;
   END IF;
END;  $$;
...
CREATE OR REPLACE FUNCTION transducer._person_phone_INSERT_fn()
   ...
   ELSE
      INSERT INTO transducer._loop VALUES (-1);
      INSERT INTO transducer._person_phone_INSERT 
      SELECT ssn, phone FROM new;
      RETURN NEW;
   END IF;
END;  $$;
...
CREATE OR REPLACE FUNCTION transducer._person_INSERT_fn()
   ...
   ELSE
      INSERT INTO transducer._loop VALUES (-1);
      INSERT INTO transducer._person_INSERT 
      SELECT ssn, name, dep_name FROM new;
      RETURN NEW;
   END IF;
END;  $$;
...
CREATE OR REPLACE FUNCTION transducer._department_INSERT_fn()
   ...
   ELSE
      INSERT INTO transducer._loop VALUES (-1);
      INSERT INTO transducer._department_INSERT 
      SELECT dep_name, dep_address FROM new;
      RETURN NEW;
   END IF;
END;  $$;
