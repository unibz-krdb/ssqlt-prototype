...
WHERE ssn IS NOT NULL AND dep_address IS NOT NULL) ON CONFLICT (city) DO NOTHING;

	INSERT INTO transducer._department_city (SELECT DISTINCT dep_address, city
FROM transducer._EMPDEP_INSERT_JOIN
NATURAL LEFT OUTER JOIN transducer._POSITION_INSERT_JOIN
WHERE ssn IS NOT NULL AND dep_address IS NOT NULL) ON CONFLICT (dep_address) DO NOTHING;

	INSERT INTO transducer._department (SELECT DISTINCT dep_name, dep_address
FROM transducer._EMPDEP_INSERT_JOIN
NATURAL LEFT OUTER JOIN transducer._POSITION_INSERT_JOIN
WHERE ssn IS NOT NULL AND dep_address IS NOT NULL) ON CONFLICT (dep_name) DO NOTHING;

	INSERT INTO transducer._person (SELECT DISTINCT ssn, name, dep_name
FROM transducer._EMPDEP_INSERT_JOIN
NATURAL LEFT OUTER JOIN transducer._POSITION_INSERT_JOIN
WHERE ssn IS NOT NULL AND dep_address IS NOT NULL) ON CONFLICT (ssn) DO NOTHING;

	INSERT INTO transducer._person_email (SELECT DISTINCT ssn, email
FROM transducer._EMPDEP_INSERT_JOIN
NATURAL LEFT OUTER JOIN transducer._POSITION_INSERT_JOIN
WHERE ssn IS NOT NULL AND dep_address IS NOT NULL) ON CONFLICT (ssn,email) DO NOTHING;

	INSERT INTO transducer._person_phone (SELECT DISTINCT ssn, phone
FROM transducer._EMPDEP_INSERT_JOIN
NATURAL LEFT OUTER JOIN transducer._POSITION_INSERT_JOIN
WHERE ssn IS NOT NULL AND dep_address IS NOT NULL) ON CONFLICT (ssn,phone) DO NOTHING;
